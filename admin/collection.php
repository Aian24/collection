...
// Release session lock ASAP - this page does heavy processing
// and holding the lock blocks ALL other requests from this user
session_write_close();
...
$selectedCollectors = isset($_GET['selectedCollectors']) && is_array($_GET['selectedCollectors']) ? $_GET['selectedCollectors'] : [];
...
if ($selectedBranch == 'Sanko Market') {
    ...
    $table = 'collected'; // Default to Sanko Market table if no branch selected
    $selectedBranch = 'Sanko Market'; // Set default branch name for display/consistency
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Count for collectors
    $collectorName = $row['collector'];
    $collectorCounts[$collectorName] = isset($collectorCounts[$collectorName]) ? $collectorCounts[$collectorName] + 1 : 1;
    ...
    $aircon = $cusa = $electricity = $water = $table_tennis = $pay_toilet = $pay_parking = $ice_water =
        ...
        $tables_chairs = $overnight_works = $vendo_sale = $zumba = $scrap = $hasang = $secdep = $utilitydep = $meterdep = $miscellaneous = $forfeited_items = 0;
    ...
    foreach ($charges as $charge) {
        ...
        if (count($matches) === 3) {
             ...
             switch ($chargeName) {
                 ...
                 case 'Scrap': $scrap += $chargeValue; break;
    ...
    // Filter by selected collectors if any are selected
    if ($includeRow && !empty($selectedCollectors)) {
        if (!in_array($row['collector'], $selectedCollectors)) {
            $includeRow = false;
        }
    }
    ...
    if ($includeRow) {
        ...
        $row_total = (float)$row['total']; // Use the database's true total to match POS exactly
...
// Fetch distinct collectors for the filter modal
$collectorCodes = [];
$sql_collector_codes = "SELECT DISTINCT collector FROM $table ORDER BY collector ASC";
$result_collector_codes = mysqli_query($conn, $sql_collector_codes);
if ($result_collector_codes) {
    while ($row_collector = mysqli_fetch_assoc($result_collector_codes)) {
        $collectorCodes[] = $row_collector['collector'];
    }
    mysqli_free_result($result_collector_codes);
} else {
    error_log("Error fetching collector codes for table $table: " . mysqli_error($conn));
}
...
if ($isAjax) {
    ...
    echo json_encode([
        ...
        'totals' => [ // Totals calculated for the filtered data
            ...
            'scrap' => number_format($total_scrap, 2),
        ...
        'collectorCodes' => $collectorCodes, // Add collector codes
        ...
        'collectorCounts' => $collectorCounts, // Add collector counts to the response
...
<body id="page-top">
    ...
    <div id="wrapper" class="blur-when-loading">
        ...
        <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <!-- Update Rent Month -->
            <li class="nav-item">
                <a class="nav-link" href="../manual_update_rentbal.php">
                    <i class="fas fa-fw fa-calendar-alt text-primary"></i>
                    <span>Update Rent Month</span>
                </a>
            </li>
        ...
        <div class="filter-bar w-100 mb-4 d-flex flex-wrap align-items-center justify-content-between" style="background: white; padding: 1.25rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-card); border: 1px solid #e5e7eb; gap: 1rem;">
            ...
            <div class="d-flex flex-wrap align-items-center" style="gap: 1rem;">
                <!-- Branch Filter -->
                <div class="filter-pill d-flex align-items-center dropdown" style="background: #f8fafc; padding: 0 16px; height: 44px; border-radius: var(--radius-md); border: 1px solid #e2e8f0; gap: 12px; cursor: pointer; justify-content: center;" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <!-- From Date -->
                <div class="filter-pill d-flex align-items-center" onclick="document.getElementById('fromDate').focus();" style="background: #f8fafc; padding: 0 16px; height: 44px; border-radius: var(--radius-md); border: 1px solid #e2e8f0; gap: 12px; cursor: pointer; justify-content: center;">
                ...
                <!-- To Date -->
                <div class="filter-pill d-flex align-items-center" onclick="document.getElementById('toDate').focus();" style="background: #f8fafc; padding: 0 16px; height: 44px; border-radius: var(--radius-md); border: 1px solid #e2e8f0; gap: 12px; cursor: pointer; justify-content: center;">
                    <i class="fas fa-calendar-alt text-gray-500"></i>
                    <div class="d-flex flex-column">
                ...
                <div class="d-flex align-items-center flex-wrap" style="gap: 8px;">
                    <button type="button" class="btn btn-info btn-sm font-weight-bold" data-toggle="modal" data-target="#spaceFilterModal" style="border-radius: var(--radius-md); padding: 0 16px; height: 44px; display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <i class="fas fa-building"></i> Spaces (<span id="selectedSpaceCount">0</span>)
                    </button>

                    <button type="button" class="btn btn-primary btn-sm font-weight-bold" data-toggle="modal" data-target="#tenantFilterModal" style="border-radius: var(--radius-md); background-color: var(--accent-indigo); border-color: var(--accent-indigo); padding: 0 16px; height: 44px; display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <i class="fas fa-users"></i> Tenants (<span id="selectedTenantCount">0</span>)
                    </button>

                    <button type="button" class="btn btn-warning btn-sm font-weight-bold" data-toggle="modal" data-target="#collectorFilterModal" style="border-radius: var(--radius-md); background-color: #f59e0b; border-color: #f59e0b; color: white; padding: 0 16px; height: 44px; display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <i class="fas fa-user-tag"></i> Collectors (<span id="selectedCollectorCount">0</span>)
                    </button>
                </div>
            </div>

            <div class="d-flex ml-auto" style="gap: 8px;">
                <button id="applyFilters" class="btn btn-primary btn-sm font-weight-bold" style="border-radius: var(--radius-md); background-color: var(--accent-indigo); border-color: var(--accent-indigo); padding: 0 16px; height: 44px; display: flex; align-items: center; justify-content: center; gap: 6px;">
                    <i class="fas fa-filter"></i> Apply
                </button>
                <button id="resetFilters" class="btn btn-light btn-sm text-secondary font-weight-bold" style="border-radius: var(--radius-md); padding: 0 16px; height: 44px; border: 1px solid #e5e7eb; display: flex; align-items: center; justify-content: center; gap: 6px;">
...
<div class="modal fade" id="tenantFilterModal" tabindex="-1" role="dialog" aria-labelledby="tenantFilterModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" id="chargeFilterBody">
                    ...
                    <div id="chargeCheckboxes" class="filter-columns-2">
                        ...
                        <label class="checkbox-container">Scrap <span class="count-badge" data-charge="Scrap">0</span>
                            <input type="checkbox" class="charge-filter" value="Scrap">
                            <span class="checkmark"></span>
                        </label>
    ...
    <!-- Update Collector Filter Modal -->
    <div class="modal fade" id="collectorFilterModal" tabindex="-1" role="dialog" aria-labelledby="collectorFilterModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="collectorFilterModalLabel">Select Collectors</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="collectorFilterBody">
                    <div class="filter-search">
                        <input type="text" class="form-control" id="collectorSearch" placeholder="Search collectors...">
                    </div>
                    <div id="collectorCheckboxes" class="filter-columns">Loading collectors...</div>
                </div>
                <div class="modal-footer">
                    <button type="button" id="selectAllCollectors" class="btn btn-outline-primary">Select All</button>
                    <button type="button" id="deselectAllCollectors" class="btn btn-outline-warning">Deselect All</button>
                    <button type="button" class="btn btn-primary" data-dismiss="modal">Done</button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var selectedSpaceFilters = [];
            var selectedCollectorFilters = []; // Add space filters array
            ...
            // Function to get selected collectors
            function getSelectedCollectors() {
                return selectedCollectorFilters;
            }
            ...
            function updateFilterCounts() {
                ...
                var selectedCollectors = selectedCollectorFilters.length;

                $('#selectedTenantCount').text(selectedTenants);
                $('#selectedChargeCount').text(selectedCharges);
                $('#selectedSpaceCount').text(selectedSpaces);
                $('#selectedCollectorCount').text(selectedCollectors);

                // Optional: Change button appearance if filters are applied
                if (selectedTenants > 0 || selectedCharges > 0 || selectedSpaces > 0 || selectedCollectors > 0) {
            ...
            $('#collectorSearch').on('input', function() {
                filterCheckboxes($(this), $('#collectorCheckboxes'));
            });
            ...
            // Function to populate the collector filter modal with checkboxes and counts
            function populateCollectorFilter(collectorCodes, collectorCounts) {
                var collectorCheckboxesHtml = '';
                
                // Get currently visible data from the table
                var tableData = dataTable.rows({search:'applied'}).data().toArray();
                
                // Filter collector codes to only show those present in current data
                var filteredCollectorCodes = [];
                if (collectorCodes && collectorCodes.length > 0) {
                    if (tableData && tableData.length > 0) {
                        // Create a set of collector codes present in current data
                        var currentCollectorCodes = new Set();
                        tableData.forEach(function(row) {
                            if (row.collector) {
                                currentCollectorCodes.add(row.collector);
                            }
                        });
                        
                        // Filter collector codes to only those present in current data
                        filteredCollectorCodes = collectorCodes.filter(function(collectorCode) {
                            return currentCollectorCodes.has(collectorCode);
                        });
                    } else {
                        filteredCollectorCodes = collectorCodes;
                    }
                    
                    filteredCollectorCodes.forEach(function(collectorCode) {
                        var count = collectorCounts[collectorCode] || 0;
                        var isChecked = selectedCollectorFilters.includes(collectorCode) ? 'checked' : '';
                        collectorCheckboxesHtml += `
                            <label class="checkbox-container">${collectorCode} <span class="count-badge">${count}</span>
                                <input type="checkbox" class="collector-filter" value="${collectorCode}" ${isChecked}>
                                <span class="checkmark"></span>
                            </label>`;
                    });
                } else {
                    collectorCheckboxesHtml = '<p>No collectors found for the selected branch and date range.</p>';
                }
                $('#collectorCheckboxes').html(collectorCheckboxesHtml);
            }
            ...
            function isColumnAllZeroOrEmpty(columnIndex, data) {
                ...
                var rawFieldMap = {
                    ...
                    34: 'raw_utilitydep',
                    35: 'raw_meterdep',
                    36: 'raw_miscellaneous',
                    37: 'raw_forfeited_items',
                    38: 'raw_total',
                    39: 'raw_rent_balance',
                    40: 'raw_rent_arrear',
                    41: 'raw_elec_arrear',
                    42: 'raw_water_arrear',
                    43: 'raw_total_arrears'
                };
            ...
            dataTable = $('#dataTable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.selectedCollectors = getSelectedCollectors(); // Add collector filters
                    },
                    dataSrc: function(json) {
                        ...
                        populateCollectorFilter(json.collectorCodes, json.collectorCounts); // Add collector filter population
                ...
                columns: [
                    ...
                    { data: 'cusa', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_cusa : data; } },
                    { data: 'electricity', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_electricity : data; } },
                    { data: 'water', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_water : data; } },
                    { data: 'table_tennis', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_table_tennis : data; } },
                    { data: 'pay_toilet', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_pay_toilet : data; } },
                    { data: 'pay_parking', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_pay_parking : data; } },
                    { data: 'ice_water', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_ice_water : data; } },
                    { data: 'ulam_vendor', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_ulam_vendor : data; } },
                    { data: 'gas', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_gas : data; } },
                    { data: 'famylihan', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_famylihan : data; } },
                    { data: 'garbage_haul', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_garbage_haul : data; } },
                    { data: 'photocopy', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_photocopy : data; } },
                    { data: 'tenant_id', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_tenant_id : data; } },
                    { data: 'function_room', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_function_room : data; } },
                    { data: 'tables_chairs', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_tables_chairs : data; } },
                    { data: 'overnight_works', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_overnight_works : data; } },
                    { data: 'vendo_sale', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_vendo_sale : data; } },
                    { data: 'zumba', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_zumba : data; } },
                    { data: 'scrap', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_scrap : data; } },
                    { data: 'hasang', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_hasang : data; } },
                     { data: 'secdep', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_secdep : data; } },
                     { data: 'utilitydep', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_utilitydep : data; } },
                     { data: 'meterdep', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_meterdep : data; } },
                     { data: 'miscellaneous', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_miscellaneous : data; } },
                     { data: 'forfeited_items', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_forfeited_items : data; } },
                    { data: 'total', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_total : data; } }, // Column for Row Total
                    { data: 'rent_balance', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_rent_balance : data; } }, // Column for Rent Balance
                    { data: 'rent_arrear', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_rent_arrear : data; } },
                    { data: 'elec_arrear', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_elec_arrear : data; } },
                    { data: 'water_arrear', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_water_arrear : data; } },
                    { data: 'total_arrears', render: function(data, type, row) { return (type === 'export' || type === 'sort') ? row.raw_total_arrears : data; } }
            ...
            // Save selected filters when modal is closed
            $('#collectorFilterModal').on('hide.bs.modal', function() {
                selectedCollectorFilters = [];
                $('#collectorFilterBody input:checked').each(function() {
                    selectedCollectorFilters.push($(this).val());
                });
                updateFilterCounts();
            });

            // Handle Select All Collectors button
            $('#selectAllCollectors').on('click', function() {
                $('#collectorFilterBody input[type="checkbox"]').prop('checked', true);
                selectedCollectorFilters = [];
                $('#collectorFilterBody input:checked').each(function() {
                    selectedCollectorFilters.push($(this).val());
                });
            ...
            // Handle Deselect All Collectors button
            $('#deselectAllCollectors').on('click', function() {
                $('#collectorFilterBody input[type="checkbox"]').prop('checked', false);
                selectedCollectorFilters = [];
                updateFilterCounts();
            });

            // Update filter counts whenever a collector checkbox state changes
            $('#collectorFilterBody').on('change', 'input[type="checkbox"]', function() {
                selectedCollectorFilters = [];
                $('#collectorFilterBody input:checked').each(function() {
                    selectedCollectorFilters.push($(this).val());
                });
