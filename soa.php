...
<body class="bg-gray-100 p-8" style="font-size: 16px;">
    <form method="POST" action="soaprint.php" class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
        <div class="mb-4">
            <label for="branchFilter" class="block text-sm font-medium text-gray-700">Select Branch</label>
            <select id="branchFilter" name="branchFilter" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                <option value="">Select Branch</option>
                <option value="Sanko Market" <?php echo (isset($_POST['branchFilter']) && $_POST['branchFilter'] == "Sanko Market") ? "selected" : ""; ?>>Sanko Market</option>
                <option value="Nova Market" <?php echo (isset($_POST['branchFilter']) && $_POST['branchFilter'] == "Nova Market") ? "selected" : ""; ?>>Nova Market</option>
                <option value="APM" <?php echo (isset($_POST['branchFilter']) && $_POST['branchFilter'] == "APM") ? "selected" : ""; ?>>APM</option>
            </select>
        </div>

        <div class="mb-4">
            <label for="tenantFilter" class="block text-sm font-medium text-gray-700">Select Tenant</label>
            <input type="text" id="tenantFilter" name="tenantFilter" placeholder="Type to search tenants..." class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="<?php echo isset($_POST['tenantFilter']) ? $_POST['tenantFilter'] : ''; ?>">
            <div id="tenantDropdown" class="hidden mt-2 border border-gray-300 rounded-md shadow-sm max-h-40 overflow-y-auto">
                </div>
        </div>

        <div class="mb-4">
            <label for="fromDateFilter" class="block text-sm font-medium text-gray-700">From Date</label>
            <input type="date" id="fromDateFilter" name="fromDateFilter" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="<?php echo isset($_POST['fromDateFilter']) ? $_POST['fromDateFilter'] : ''; ?>">
        </div>

        <div class="mb-4">
            <label for="toDateFilter" class="block text-sm font-medium text-gray-700">To Date</label>
            <input type="date" id="toDateFilter" name="toDateFilter" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="<?php echo isset($_POST['toDateFilter']) ? $_POST['toDateFilter'] : ''; ?>">
        </div>

        <button type="submit" class="w-full bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600">Generate Statement</button>
    </form>

    <script>
        $(document).ready(function () {
            // Fetch tenant names from the server based on selected branch
            function fetchTenants() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#tenantDropdown').removeClass('hidden');
                    },
                    error: function () {
            ...
            $('#tenantDropdown').on('click', 'option', function () {
                $('#tenantFilter').val($(this).text());
                $('#tenantDropdown').addClass('hidden');
            });
